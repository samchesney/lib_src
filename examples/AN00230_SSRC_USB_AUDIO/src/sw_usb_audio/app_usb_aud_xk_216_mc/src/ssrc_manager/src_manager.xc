...
int g_samps_from_host[SSRC_N_INSTANCES][SSRC_N_IN_SAMPLES][SSRC_CHANNELS_PER_INSTANCE];     //Single buffers for input
int g_samps_to_i2s[NUM_USB_CHAN_OUT][SSRC_OUT_BUFF_SIZE * 2];            //Circular buffers for output
...
static inline void init_fifos(){
    unsafe{
        for (int i=0 ; i<NUM_USB_CHAN_OUT; i++){
            ...
            ptr_wr_samps_to_i2s[i]   = (ptr_base_samps_to_i2s[i] + SSRC_OUT_BUFF_SIZE / 2);
            memset(ptr_base_samps_to_i2s[i], 0, SSRC_OUT_BUFF_SIZE * sizeof(int));
        }
    }
}
...
static inline unsigned push_single_sample_to_fifo(int samp, int * unsafe * unsafe wr_ptr, int * unsafe rd_ptr, int * unsafe fifo_base){
    ...
    unsafe{
        **wr_ptr = samp;    //write value into fifo
        ...
        if (*wr_ptr >= fifo_base + SSRC_OUT_BUFF_SIZE) *wr_ptr = fifo_base; //wrap pointer
        if (*wr_ptr == rd_ptr) {
            success = 0;
        }
    }
...
static inline unsigned pull_sample_from_fifo(int &samp, int * unsafe wr_ptr, int * unsafe  * unsafe rd_ptr, int * unsafe fifo_base){
    ...
    unsafe{
        ...
        if (*rd_ptr >= fifo_base + SSRC_OUT_BUFF_SIZE) *rd_ptr = fifo_base; //wrap pointer
        if (*rd_ptr == wr_ptr){
            success = 0;
        }
    }
...
static inline unsigned trigger_dsp(streaming chanend c_dsp[SSRC_N_INSTANCES],
        int samps_from_host[SSRC_N_INSTANCES][SSRC_N_IN_SAMPLES][SSRC_CHANNELS_PER_INSTANCE], unsigned sample_rate_code)
{
    unsigned pull_count;
#pragma loop unroll
    for (int i=0; i<SSRC_N_INSTANCES; i++) c_dsp[i] <: sample_rate_code;//Send sample rate control token initially

#pragma loop unroll
    for (int i=0; i<SSRC_N_INSTANCES; i++){                             //Send frame of samples out to DSP engine
#pragma loop unroll
        for (int j=0; j<SSRC_N_IN_SAMPLES; j++){                    //Send to each engine
#pragma loop unroll
            for (int k=0; k<SSRC_CHANNELS_PER_INSTANCE; k++){            //Send number of samples handled by each engine
...
#pragma loop unroll
    for (int i=0; i<SSRC_N_INSTANCES; i++) c_dsp[i] :> pull_count;      //Get number of samples to receive

#pragma loop unroll
    for (int i=0; i<SSRC_N_INSTANCES; i++){                             //From each engine..
        unsigned success = 1;
        for (int j=0; j<pull_count; j++){                           //Get n samples
#pragma loop unroll
            for (int k=0; k<SSRC_CHANNELS_PER_INSTANCE; k++){            //..for each channel handled by engine
                int samp;
                c_dsp[i] :> samp;
                unsigned channel_no = k + i * SSRC_CHANNELS_PER_INSTANCE;
...
void src_manager_task(chanend c_host, chanend c_i2s, streaming chanend c_dsp[SSRC_N_INSTANCES], chanend c_i2s_sampfreq)
{
...
#pragma loop unroll
                for (int chan_i=0; chan_i<NUM_USB_CHAN_OUT; chan_i++){  //Get samples from host
                    ...
                    g_samps_from_host[chan_i/SSRC_CHANNELS_PER_INSTANCE]
                                     [samples_idx]
                                     [chan_i % SSRC_CHANNELS_PER_INSTANCE] = samp;
                }
            }
...
//There may be multiple instances of this function accoring to the value of SSRC_N_INSTANCES. See par{} below
void dsp_task(streaming chanend c_dsp, unsigned instance_id)
{
    ...
    int        out_buff[SSRC_OUT_BUFF_SIZE];
    ...
    memset(out_buff, 0, SSRC_OUT_BUFF_SIZE * sizeof(int));
    //printstrln("DSP_task");
    ...
    while(1){
        ...
        for(unsigned uj = 0; uj < n_samps_out * SSRC_CHANNELS_PER_INSTANCE; uj++)
        {
            int tmp;
            tmp = out_buff[uj];
            c_dsp <: tmp;
...
void src_manager(chanend c_host, chanend c_i2s, chanend c_i2s_sampfreq){
    streaming chan c_dsp[SSRC_N_INSTANCES];
    par
    {
        src_manager_task(c_host, c_i2s, c_dsp, c_i2s_sampfreq);         //Buffer manager
        par (int i=0; i < SSRC_N_INSTANCES; i++) dsp_task(c_dsp[i], i);     //DSP Engines
    }
}
